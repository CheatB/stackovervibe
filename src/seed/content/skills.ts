...
export const enrichedSkills: EnrichedContent[] = [
  ...
  ───────────────────────────────────────────── */
  {
    ...
    description: root([
      ...
      list(
        [
          ...
          "Генерируется структура: `bot/handlers/`, `bot/keyboards/`, `bot/states/`, `bot/middlewares/`",
          "Создаются хендлеры для каждого сценария с FSM-состояниями и валидацией ввода",
          "Настраивается middleware: логирование, rate-limit, авторизация по ролям",
          "Добавляется `docker-compose.yml` + `.env.example` + `alembic/` для миграций",
          "Пишутся pytest-тесты для хендлеров и FSM-переходов",
        ],
        true,
      ),
      ...
      paragraph("Примеры промптов, которые активируют этот скилл:"),
      codeBlock(
      ...
      hr(),
      heading(2, "Пример структуры проекта"),
      codeBlock(
      ...
      heading(3, "Поддерживает ли aiogram v2?"),
      paragraph(
    ...
    seoDescription:
      "Скилл aiogram-bot: структура проекта, FSM-диалоги, клавиатуры, PostgreSQL. Создание Telegram-ботов с Claude Code.",
  },
