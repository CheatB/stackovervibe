...
export const enrichedCommands: EnrichedContent[] = [
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    description: root([
      ...
      paragraph(
        "У тебя есть идея, но непонятно как превратить её в работающий сайт или приложение? Напиши `/new-project` — и Claude сделает всё сам. От пустого сервера до готового продукта с доменом и SSL.",
      ),
      paragraph(
        "Ты просто отвечаешь на вопросы: что за проект, для кого, какие функции. Дальше Claude пишет код, настраивает сервер и запускает. Через пару часов — рабочий сайт.",
      ),
      hr(),
...
Фаза: 9 | Следующее: деплой`,
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    description: root([
      heading(2, "Простым языком"),
      paragraph(
        "У тебя копится список идей и задач — одни важные, другие «было бы неплохо». Напиши `/backlog` — увидишь всё что запланировано, отсортированное по важности. А `/backlog add` добавит новую задачу за секунды, чтобы не забыть на ходу.",
      ),
      paragraph(
        "Claude умеет расставлять приоритеты по специальной формуле: учитывает сколько людей затронет задача, насколько улучшит проект и сколько времени займёт. Больше не нужно гадать что делать первым.",
      ),
      hr(),
      heading(2, "Что делает"),
      paragraph(
...
# Строгий режим tsconfig.json:
{
  ...
  // ─────────────────────────────────────────────────────────
  {
    ...
    description: root([
      heading(2, "Простым языком"),
      paragraph(
        "Купил новый сервер — он пустой, нет ничего. Чтобы Claude работал на нём с полным набором инструментов (автопроверки, форматирование, напоминания), нужно всё настроить. `/vps-setup-framework` делает это автоматически за 5 минут.",
      ),
      paragraph(
        "Напиши `/vps-setup-framework` с адресом сервера — Claude подключится и установит всё нужное. После этого на новом сервере всё работает точно так же как на основном. Как развернуть рабочее место в новом офисе одной кнопкой.",
      ),
      hr(),
      heading(2, "Что делает"),
      paragraph(
