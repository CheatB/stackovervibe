...
/** Получить фреймворки с пагинацией */
export async function getFrameworks(страница = 1, лимит = 20) {
  const payload = await getPayloadClient();
  return payload.find({
...
/** Популярные фреймворки (для сайдбара) */
export async function getPopularFrameworks(лимит = 5) {
  const payload = await getPayloadClient();
  const { docs } = await payload.find({
    collection: "frameworks",
    where: { status: { equals: "published" } },
...
/** Получить фреймворки пользователя */
export async function getUserFrameworks(userId: number | string) {
  const payload = await getPayloadClient();
  const { docs } = await payload.find({
    collection: "frameworks",
    where: {
...
export async function getFeedPage(options: {
  ...
  const нужныФреймворки = type === "all" || type === "framework";
        ...
        .then(({ docs }) =>
          docs.map((ф): FeedItem => {
            ...
            return {
              ...
              votes: ((ф.likes as number) || 0) - ((ф.dislikes as number) || 0),
              ...
              author: автор
                ...
                : null,
              publishedAt: (ф.publishedAt as string) || (ф.createdAt as string),
              hasAcceptedAnswer: false,
            };
          }),
        ),
  ...
  if (sort === "hot") {
    ...
    результаты.sort((a, b) => {
      const scoreA =
        (a.votes * 2 + a.answersCount * 3 + a.views) /
        ((сейчас - new Date(a.publishedAt).getTime()) / 3600000 + 2);
      const scoreB =
        (b.votes * 2 + b.answersCount * 3 + b.views) /
        ((сейчас - new Date(b.publishedAt).getTime()) / 3600000 + 2);
...
export async function getSiteStats() {
  ...
  const [гайды, инструменты, вопросы, посты, фреймворки] = await Promise.all([
    payload.count({
      collection: "guides",
      where: { status: { equals: "published" } },
    }),
