...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [гайды, инструменты, страницы, вопросы, посты, теги] = await Promise.all([
    payload.find({
      collection: 'guides',
  ...
  return [...статичные, ...гайдыUrlы, ...инструментыUrlы, ...вопросыUrlы, ...постыUrlы, ...тегиUrlы, ...страницыUrlы]
}
