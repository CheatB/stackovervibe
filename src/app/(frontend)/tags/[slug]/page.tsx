...
export async function generateMetadata({
  ...
  const оптимизирован = !!(тег as any).seoDescription;

  return {
    ...generatePageMetadata({
      title: (тег as any).seoTitle || `Тег: ${тег.title}`,
      description:
        (тег as any).seoDescription ||
        (тег as any).description ||
        `Контент с тегом "${тег.title}" на Stackovervibe`,
      url: `/tags/${тег.slug}`,
    }),
    /* noindex для неоптимизированных тегов — тонкие страницы */
    ...(!оптимизирован ? { robots: { index: false, follow: true } } : {}),
  };
}
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-xs text-[var(--color-text-muted)] font-[family-name:var(--font-code)] mt-2">
          {отфильтрованные.length}{" "}
          {отфильтрованные.length === 1 ? "запись" : "записей"}
        </p>
      </div>
