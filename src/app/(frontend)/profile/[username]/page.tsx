...
import { getUserByUsername, getUserPosts } from '@/lib/payload'
...
export async function generateMetadata({ params }: ПараметрыСтраницы): Promise<Metadata> {
  ...
  return {
    ...
    description: пользователь.bio || `Профиль ${пользователь.displayName || username} на Stackovervibe`,
  }
}
...
export default async function ProfilePage({ params }: ПараметрыСтраницы) {
  ...
  const посты = await getUserPosts(пользователь.id)
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      {/* Профиль */}
      <div className="flex items-start gap-6 p-6 border border-[var(--color-border)] rounded-lg bg-[var(--color-bg-card)]">
        ...
        <div className="flex-1">
          <h1 className="text-2xl mb-1">{пользователь.displayName || `@${username}`}</h1>
          ...
          {пользователь.bio && (
            <p className="text-sm text-[var(--color-text-muted)] mt-2">{пользователь.bio}</p>
          )}
          <div className="flex gap-4 mt-3 text-sm font-[family-name:var(--font-code)]">
            <span>
              <span className="text-[var(--color-primary)]">{посты.length}</span>{' '}
