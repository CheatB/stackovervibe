...
### Фаза 12: Первый фреймворк + fix рендера — ГОТОВО
- [x] 12.1 create-framework.ts — скрипт создания фреймворка через Payload local API
- [x] 12.2 Фреймворк "Вайбкодинг с нуля" — 7 фаз, 9 code-блоков, 5 тегов
- [x] 12.3 Fix RichTextRenderer — поддержка стандартных Lexical code-нод (type:"code")
- [x] 12.4 Деплой на прод — 200 OK, 9/9 code-блоков рендерятся
